/**
 * Standalone draft jobs section for a specific source type.
 * Each instance owns fully independent state — no shared state across sections.
 */
...
function getStepState(
  draft: DraftJob, stepAction: string, tsField: keyof DraftJob, busyAction: string | undefined
): { state: StepState; tooltip: string } {
  if (busyAction === stepAction || busyAction === 'ai-run-all') {
    if (busyAction === 'ai-run-all' && draft[tsField]) return { state: 'completed', tooltip: `Completed: ${draft[tsField]}` };
    if (busyAction === stepAction || (busyAction === 'ai-run-all' && !draft[tsField])) return { state: 'running', tooltip: 'Running...' };
  }
  if (draft[tsField]) return { state: 'completed', tooltip: `Done: ${new Date(draft[tsField] as string).toLocaleString()}` };
  const log = draft.ai_enrichment_log;
  if (log && Array.isArray(log)) {
    for (let i = log.length - 1; i >= 0; i--) {
      const entry = log[i] as any;
      if (entry?.action === stepAction) {
        if (entry.status === 'failed') return { state: 'failed', tooltip: `Failed: ${entry.error || 'Unknown error'}` };
        if (entry.status === 'skipped') return { state: 'skipped', tooltip: `Skipped: ${entry.reason || 'Status guard or protection'}` };
        break;
      }
    }
  }
...
const STEP_ICONS: Record<StepState, typeof Circle> = {
  pending: Circle, running: Loader2, completed: CheckCircle, failed: XCircle, skipped: Ban,
};

interface BulkProgress {
  total: number; current: number; currentTitle: string; succeeded: number; failed: number; skipped: number;
}

interface BulkResult {
  id: string; title: string; success: boolean; error?: string; warning?: string;
}

type FieldFixCandidate = Pick<DraftJob, 'id' | 'title' | 'status' | 'dedup_status' | 'fields_missing' | 'organization_name' | 'post_name' | 'location' | 'state' | 'city' | 'category' | 'department' | 'qualification' | 'application_mode' | 'official_apply_url' | 'official_notification_url' | 'ai_fix_missing_at'>;
type BulkRunCandidate = Pick<DraftJob, 'id' | 'title' | 'status' | 'dedup_status' | 'ai_clean_at' | 'ai_enrich_at' | 'ai_links_at' | 'ai_fix_missing_at' | 'ai_seo_at'>;
...
const ACTIONABLE_FIELD_FIX_KEYS = ['title', 'organization_name', 'post_name', 'location', 'state', 'city', 'category', 'department', 'qualification', 'application_mode'] as const;
...
const draftNeedsFieldFix = (draft: FieldFixCandidate) => {
  ...
  return ACTIONABLE_FIELD_FIX_KEYS.some(f => !hasValue(draft[f])) || (!hasValue(draft.official_apply_url) && !hasValue(draft.official_notification_url));
};

const draftNeedsRunAll = (draft: BulkRunCandidate) => {
  if (draft.status !== 'draft') return false;
  if (draft.dedup_status === 'duplicate') return false;
  return [draft.ai_clean_at, draft.ai_enrich_at, draft.ai_links_at, draft.ai_fix_missing_at, draft.ai_seo_at].some(v => !v);
};

// ── Filter tab definitions ──
type GeneralFilter = 'all' | 'draft' | 'enriched' | 'reviewed' | 'approved' | 'promoted' | 'duplicate' | 'rejected';
type GovtFilter = 'govt-all' | 'govt-ready' | 'govt-review' | 'govt-incomplete' | 'govt-retry' | 'govt-auto-eligible' | 'govt-failed' | 'govt-published' | 'govt-no-dates' | 'govt-no-links' | 'govt-low-conf';
type FilterTab = GeneralFilter | GovtFilter;

const GENERAL_TABS: { key: GeneralFilter; label: string }[] = [
  { key: 'all', label: 'All' }, { key: 'draft', label: 'Draft' }, { key: 'enriched', label: 'Enriched' },
  { key: 'reviewed', label: 'Reviewed' }, { key: 'approved', label: 'Approved' }, { key: 'promoted', label: 'Published' },
  { key: 'duplicate', label: 'Duplicates' }, { key: 'rejected', label: 'Rejected' },
];

const GOVT_TABS: { key: GovtFilter; label: string }[] = [
  { key: 'govt-all', label: 'All' }, { key: 'govt-ready', label: 'Ready' }, { key: 'govt-review', label: 'Review' },
  { key: 'govt-incomplete', label: 'Incomplete' }, { key: 'govt-retry', label: 'Retry' },
  { key: 'govt-auto-eligible', label: 'Auto-Eligible' }, { key: 'govt-failed', label: 'Failed' },
  { key: 'govt-published', label: 'Published' }, { key: 'govt-no-dates', label: 'No Dates' },
  { key: 'govt-no-links', label: 'No Links' }, { key: 'govt-low-conf', label: 'Low Conf' },
];
...
export function DraftJobsSection({ sourceTypeTag }: DraftJobsSectionProps) {
  ...
  const [activeFilter, setActiveFilter] = useState<FilterTab>(isGovt ? 'govt-all' : 'all');
  ...
  // Govt auto-publish state (only used when isGovt)
  ...
  const fetchFieldFixCandidates = useCallback(async (): Promise<FieldFixCandidate[]> => {
    ...
    while (true) {
      const { data, error } = await supabase
        ...
        .eq('source_type_tag', sourceTypeTag)
        .in('status', ['draft', 'enriched'])
        .neq('dedup_status', 'duplicate')
        .order('created_at', { ascending: false })
        .range(from, from + pageSize - 1);
      if (error) throw error;
      rows.push(...((data as unknown as FieldFixCandidate[]) || []).filter(draftNeedsFieldFix));
  ...
  const fetchBulkRunCandidates = useCallback(async (): Promise<BulkRunCandidate[]> => {
    ...
    while (true) {
      const { data, error } = await supabase
        ...
        .range(from, from + pageSize - 1);
      if (error) throw error;
      rows.push(...((data as unknown as BulkRunCandidate[]) || []).filter(draftNeedsRunAll));
      if (!data || data.length < pageSize) break;
      from += pageSize;
    }
    return rows;
  ...
  const fetchImageCandidates = useCallback(async (): Promise<ImageCandidate[]> => {
    ...
    while (true) {
      const { data, error } = await supabase
        ...
        .range(from, from + pageSize - 1);
      if (error) throw error;
      rows.push(...((data as unknown as ImageCandidate[]) || []).filter(d => !d.cover_image_url?.trim()));
      if (!data || data.length < pageSize) break;
      from += pageSize;
    }
    return rows;
  ...
  const fetchDrafts = useCallback(async () => {
    ...
    // Apply filter
    if (isGovt) {
      if (activeFilter === 'govt-ready') query = query.eq('publish_readiness', 'ready');
      else if (activeFilter === 'govt-review') query = query.eq('publish_readiness', 'review_needed');
      else if (activeFilter === 'govt-incomplete') query = query.eq('publish_readiness', 'incomplete');
      else if (activeFilter === 'govt-retry') query = query.eq('publish_readiness', 'retry');
      else if (activeFilter === 'govt-no-dates') query = query.is('closing_date', null).is('last_date_of_application', null);
      else if (activeFilter === 'govt-no-links') query = query.is('official_apply_url', null).is('official_notification_url', null);
      else if (activeFilter === 'govt-low-conf') query = query.not('field_confidence', 'eq', '{}');
      else if (activeFilter === 'govt-auto-eligible') query = query.eq('auto_publish_eligible', true).neq('status', 'promoted');
      else if (activeFilter === 'govt-failed') query = query.eq('publish_readiness', 'failed');
      else if (activeFilter === 'govt-published') query = query.eq('status', 'promoted');
      // govt-all: no additional filter
    } else {
    ...
    const [{ data, error }, fieldFixResult, bulkRunResult, imageResult] = await Promise.all([
      query,
      fetchFieldFixCandidates().then(r => ({ data: r, error: null as Error | null }), e => ({ data: [] as FieldFixCandidate[], error: e as Error })),
      fetchBulkRunCandidates().then(r => ({ data: r, error: null as Error | null }), e => ({ data: [] as BulkRunCandidate[], error: e as Error })),
      fetchImageCandidates().then(r => ({ data: r, error: null as Error | null }), e => ({ data: [] as ImageCandidate[], error: e as Error })),
    ]);

    if (error) toast({ title: 'Error loading drafts', description: error.message, variant: 'destructive' });
    else setDrafts((data as unknown as DraftJob[]) || []);

    if (fieldFixResult.error) { toast({ title: 'Error loading field-fix counts', description: fieldFixResult.error.message, variant: 'destructive' }); setFieldFixCandidates([]); }
    else setFieldFixCandidates(fieldFixResult.data);
  ...
  const runAiAction = async (draftId: string, action: AiAction) => {
    ...
    try {
      ...
      toast({
        ...
        description: action === 'ai-run-all' ? `${data.succeeded}/${data.total} steps succeeded` : data.message || 'Done',
      });
      await fetchDrafts();
    } catch (e: any) {
  ...
  // ── Bulk Run All ──
  const getEligibleDrafts = useCallback(() => bulkRunCandidates.filter(d => !busyRows[d.id]), [bulkRunCandidates, busyRows]);

  const runBulkAll = async () => {
    const eligible = getEligibleDrafts();
    if (eligible.length === 0) { toast({ title: 'No eligible rows', description: 'No draft rows available for bulk processing.' }); return; }
    if (!window.confirm(`Run all AI steps on ${eligible.length} eligible draft row(s)?\n\nThis processes rows sequentially and may take several minutes.`)) return;

    bulkCancelRef.current = false;
    setBulkRunning(true);
    setBulkResults(null);
    const results: BulkResult[] = [];
    const progress: BulkProgress = { total: eligible.length, current: 0, currentTitle: '', succeeded: 0, failed: 0, skipped: 0 };
    setBulkProgress({ ...progress });

    for (let i = 0; i < eligible.length; i++) {
      if (bulkCancelRef.current) {
        for (let j = i; j < eligible.length; j++) { results.push({ id: eligible[j].id, title: eligible[j].title || 'Untitled', success: false, error: 'Cancelled' }); progress.skipped++; }
        break;
      }
      ...
      while (attempt < 3 && !succeeded) {
        attempt++;
        try {
          const { data, error } = await supabase.functions.invoke('firecrawl-ai-enrich', { body: { action: 'ai-run-all', draft_id: draft.id, aiModel: selectedModel } });
          if (error) throw new Error(error.message);
          if (data?.error) {
            if (data.error.toLowerCase().includes('rate limit') && attempt < 3) { await new Promise(r => setTimeout(r, attempt * 15000)); continue; }
          ...
          if (e.message?.toLowerCase().includes('rate limit') && attempt < 3) { await new Promise(r => setTimeout(r, attempt * 15000)); continue; }
    ...
    await fetchDrafts();
    toast({ title: 'Bulk Run All complete', description: `✅ ${progress.succeeded} succeeded · ❌ ${progress.failed} failed · ⏭ ${progress.skipped} skipped` });
  };
  ...
  const runDedup = async () => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke('firecrawl-ingest', { body: { action: 'dedup-drafts' } });
      if (error) throw new Error(error.message);
      if (data?.error) throw new Error(data.error);
      toast({ title: 'Dedup complete', description: `Checked: ${data.checked || 0}, Duplicates: ${data.duplicatesFound || 0}` });
      await fetchDrafts();
    } catch (e: any) {
  ...
  const runPurgeHighDuplicates = async () => {
    setPurging(true);
    try {
      const { data, error } = await supabase.functions.invoke('firecrawl-ingest', { body: { action: 'purge-high-duplicates' } });
      if (error) throw new Error(error.message);
      if (data?.error) throw new Error(data.error);
      toast({ title: 'Purge complete', description: data.message || `Deleted ${data.deleted || 0} high-confidence duplicate(s).` });
      await fetchDrafts();
    } catch (e: any) {
  ...
  // ── Per-row Image ──
  const hasExistingImage = (draft: DraftJob): boolean => !!(draft.cover_image_url && draft.cover_image_url.trim().length > 0);

  const createImage = async (draftId: string) => {
    const draft = drafts.find(d => d.id === draftId);
    if (!draft) return;
    if (hasExistingImage(draft)) { toast({ title: 'Image exists', description: 'This row already has a cover image. Skipping.' }); return; }
    setBusyRows(prev => ({ ...prev, [draftId]: 'ai-cover-image' }));
    try {
      const { data: promptData, error: promptErr } = await supabase.functions.invoke('firecrawl-ai-enrich', { body: { action: 'ai-cover-prompt', draft_id: draftId, aiModel: selectedModel } });
      if (promptErr) throw new Error(promptErr.message);
      if (promptData?.error) throw new Error(promptData.error);
      const { data, error } = await supabase.functions.invoke('firecrawl-ai-enrich', { body: { action: 'ai-cover-image', draft_id: draftId, imageModel: selectedImageModel } });
      if (error) throw new Error(error.message);
      if (data?.error) throw new Error(data.error);
    ...
    } finally { setBusyRows(prev => { const n = { ...prev }; delete n[draftId]; return n; }); }
  };

  // ── Bulk Images ──
  const getDraftsNeedingImages = useCallback(() => imageCandidates.filter(d => !busyRows[d.id]), [imageCandidates, busyRows]);

  const runBulkImages = async () => {
    const eligible = getDraftsNeedingImages();
    if (eligible.length === 0) { toast({ title: 'No rows need images' }); return; }
    if (!window.confirm(`Generate cover images for ${eligible.length} row(s) without images?`)) return;
    ...
    for (let i = 0; i < eligible.length; i++) {
      if (bulkImageCancelRef.current) { progress.skipped += eligible.length - i; break; }
      const draft = eligible[i];
      progress.current = i + 1;
      progress.currentTitle = draft.title || 'Untitled';
      setBulkImageProgress({ ...progress });

      const { data: freshDraft } = await supabase.from('firecrawl_draft_jobs').select('cover_image_url').eq('id', draft.id).single();
      if (freshDraft?.cover_image_url && freshDraft.cover_image_url.trim().length > 0) { progress.skipped++; setBulkImageProgress({ ...progress }); continue; }
      ...
      while (attempt < 3 && !done) {
        attempt++;
        try {
          await supabase.functions.invoke('firecrawl-ai-enrich', { body: { action: 'ai-cover-prompt', draft_id: draft.id, aiModel: selectedModel } });
          const { data, error } = await supabase.functions.invoke('firecrawl-ai-enrich', { body: { action: 'ai-cover-image', draft_id: draft.id, imageModel: selectedImageModel } });
          if (error) throw new Error(error.message);
          if (data?.error) { if (data.error.toLowerCase().includes('rate limit') && attempt < 3) { await new Promise(r => setTimeout(r, attempt * 15000)); continue; } throw new Error(data.error); }
          progress.succeeded++; done = true;
        } catch (e: any) {
          if (e.message?.toLowerCase().includes('rate limit') && attempt < 3) { await new Promise(r => setTimeout(r, attempt * 15000)); continue; }
          progress.failed++; done = true;
        }
      }
      setBusyRows(prev => { const n = { ...prev }; delete n[draft.id]; return n; });
      setBulkImageProgress({ ...progress });
    ...
    await fetchDrafts();
    toast({ title: 'Bulk Image Generation complete', description: `✅ ${progress.succeeded} created · ❌ ${progress.failed} failed · ⏭ ${progress.skipped} skipped` });
  };

  // ── Bulk Fix Fields ──
  const getDraftsNeedingFieldFix = useCallback(() => fieldFixCandidates.filter(d => !busyRows[d.id]), [fieldFixCandidates, busyRows]);

  const runBulkFixFields = async () => {
    const eligible = getDraftsNeedingFieldFix();
    if (eligible.length === 0) { toast({ title: 'No rows need field fixes' }); return; }
    if (!window.confirm(`Fix missing fields on ${eligible.length} row(s) using AI?`)) return;
    ...
    for (let i = 0; i < eligible.length; i++) {
      if (bulkFixFieldsCancelRef.current) { progress.skipped += eligible.length - i; break; }
      const draft = eligible[i];
      progress.current = i + 1;
      progress.currentTitle = draft.title || 'Untitled';
      setBulkFixFieldsProgress({ ...progress });
      ...
      while (attempt < 3 && !done) {
        attempt++;
        try {
          const { data, error } = await supabase.functions.invoke('firecrawl-ai-enrich', { body: { action: 'ai-fix-fields', draft_id: draft.id, aiModel: selectedModel } });
          if (error) throw new Error(error.message);
          if (data?.error) { if (data.error.toLowerCase().includes('rate limit') && attempt < 3) { await new Promise(r => setTimeout(r, attempt * 15000)); continue; } throw new Error(data.error); }
          if (data?.no_changes) progress.skipped++; else progress.succeeded++;
          done = true;
        } catch (e: any) {
          if (e.message?.toLowerCase().includes('rate limit') && attempt < 3) { await new Promise(r => setTimeout(r, attempt * 15000)); continue; }
          progress.failed++; done = true;
    ...
    await fetchDrafts();
    toast({ title: 'Bulk Fix Fields complete', description: `✅ ${progress.succeeded} fixed · ❌ ${progress.failed} failed · ⏭ ${progress.skipped} unchanged` });
  };
  ...
  const updateStatus = async (draftId: string, newStatus: string) => {
    if (newStatus === 'approved') {
      try {
        const { data, error } = await supabase.functions.invoke('firecrawl-ingest', { body: { action: 'validate-for-approval', draft_id: draftId } });
        if (error) throw new Error(error.message);
        if (data?.error) throw new Error(data.error);
        if (!data.can_approve) {
          toast({ title: 'Cannot approve', description: `Blocking issues: ${data.errors?.join('; ') || 'Unknown'}`, variant: 'destructive' });
          if (data.warnings?.length > 0) toast({ title: 'Warnings', description: data.warnings.join('; ') });
          return;
        }
        if (data.warnings?.length > 0) toast({ title: 'Approved with warnings', description: data.warnings.join('; ') });
      } catch (e: any) { toast({ title: 'Validation error', description: e.message, variant: 'destructive' }); return; }
    }
    const { data: { user } } = await supabase.auth.getUser();
    const { error } = await supabase.from('firecrawl_draft_jobs').update({ status: newStatus, reviewed_at: new Date().toISOString(), reviewed_by: user?.id || null } as any).eq('id', draftId);
    if (error) toast({ title: 'Error', description: error.message, variant: 'destructive' });
    else { toast({ title: `Status → ${newStatus}` }); await fetchDrafts(); }
  };
  ...
  const validateForPublish = (draft: DraftJob) => {
    ...
    if ((draft as any).tp_clean_status !== 'cleaned') errors.push('First Run the Third Party Cleaner');
    ...
    if (draft.status !== 'approved' && draft.status !== 'enriched' && draft.status !== 'reviewed') warnings.push(`Status is "${draft.status}" — typically rows are approved before publishing`);
  ...
  const executePublish = async (draft: DraftJob) => {
    ...
    try {
      ...
      const { error } = await supabase.from('employment_news_jobs').insert({
      ...
      if (error) throw new Error(error.message);
      await supabase.from('firecrawl_draft_jobs').update({ status: 'promoted' } as any).eq('id', draft.id);
  ...
  const filterTabs = isGovt ? GOVT_TABS : GENERAL_TABS;
    ...
    <>
      <Card>
        <CardHeader className="pb-3">
          <div className="flex flex-col gap-2">
            <CardTitle className="text-lg flex items-center gap-2">
              ...
              {TITLE_MAP[sourceTypeTag]}
            </CardTitle>
            <div className="flex flex-wrap items-center gap-2">
              <div className="flex items-center gap-1.5">
                <span className="text-[10px] text-muted-foreground font-medium">Text:</span>
                <AiModelSelector value={selectedModel} onValueChange={setSelectedModel} capability="text" size="sm" allowedValues={[...SEO_FIX_MODEL_VALUES]} />
              </div>
              <div className="flex items-center gap-1.5">
                <span className="text-[10px] text-muted-foreground font-medium">Image:</span>
                <AiModelSelector value={selectedImageModel} onValueChange={setSelectedImageModel} capability="image" size="sm" />
              </div>

              {/* Shared bulk actions */}
              {bulkRunning ? (
                <Button variant="destructive" size="sm" onClick={() => { bulkCancelRef.current = true; }}><X className="h-3.5 w-3.5 mr-1.5" />Stop Run All</Button>
              ) : (
                <Button variant="default" size="sm" onClick={runBulkAll} disabled={eligibleCount === 0 || loading}>
                  <Zap className="h-3.5 w-3.5 mr-1.5" />Bulk Run All{eligibleCount > 0 ? ` (${eligibleCount})` : ''}
                </Button>
              )}
              {bulkImageRunning ? (
                <Button variant="destructive" size="sm" onClick={() => { bulkImageCancelRef.current = true; }}><X className="h-3.5 w-3.5 mr-1.5" />Stop Images</Button>
              ) : (
                <Button variant="outline" size="sm" onClick={runBulkImages} disabled={imageEligibleCount === 0 || loading}>
                  <Image className="h-3.5 w-3.5 mr-1.5" />Bulk Images{imageEligibleCount > 0 ? ` (${imageEligibleCount})` : ''}
                </Button>
              )}
              {bulkFixFieldsRunning ? (
                <Button variant="destructive" size="sm" onClick={() => { bulkFixFieldsCancelRef.current = true; }}><X className="h-3.5 w-3.5 mr-1.5" />Stop Fix Fields</Button>
              ) : (
                <Button variant="outline" size="sm" onClick={runBulkFixFields} disabled={fixFieldsEligibleCount === 0 || loading}>
                  <AlertTriangle className="h-3.5 w-3.5 mr-1.5" />Bulk Fix Fields{fixFieldsEligibleCount > 0 ? ` (${fixFieldsEligibleCount})` : ''}
                </Button>
              )}
              <Button variant="outline" size="sm" onClick={runDedup} disabled={dedupRunning}>
                {dedupRunning ? <Loader2 className="h-3.5 w-3.5 animate-spin mr-1.5" /> : <ShieldAlert className="h-3.5 w-3.5 mr-1.5" />}Dedup
              </Button>
              <Button variant="destructive" size="sm" onClick={runPurgeHighDuplicates} disabled={purging || dedupRunning} title="Delete all high-confidence duplicate rows">
                {purging ? <Loader2 className="h-3.5 w-3.5 animate-spin mr-1.5" /> : <Trash2 className="h-3.5 w-3.5 mr-1.5" />}Purge Duplicates
              </Button>
              {/* Bulk TP Cleaner */}
              <Button
                ...
                onClick={async () => {
                  ...
                  if (uncleaned.length === 0) { toast({ title: 'All clean', description: 'No rows need third-party cleaning.' }); return; }
                  setTpCleanerRunning(true);
                  setTpCleanerReport(null);
                  toast({ title: 'TP Cleaner', description: `Cleaning ${uncleaned.length} rows...` });
                  try {
                    const { data, error } = await supabase.functions.invoke('firecrawl-cleanup-branding', { body: { action: 'clean-batch', draft_ids: uncleaned.map(d => d.id) } });
                    if (error) throw error;
                    setTpCleanerReport({ cleaned: data?.cleaned || 0, failed: data?.failed || 0, total: uncleaned.length, failedRows: (data?.failed_details || []).map((d: any) => d?.title || d?.id || 'Unknown'), timestamp: new Date().toLocaleString() });
                    await fetchDrafts();
                  } catch (e: any) {
                    setTpCleanerReport({ cleaned: 0, failed: uncleaned.length, total: uncleaned.length, failedRows: [e.message], timestamp: new Date().toLocaleString() });
                  } finally { setTpCleanerRunning(false); }
                }}
              >
              ...
              {/* Government-only bulk actions */}
              {isGovt && (
                <>
                  <Button
                    variant="default" size="sm" disabled={govtPublishRunning || loading}
                    onClick={async () => {
                      setGovtPublishRunning(true);
                      setGovtPublishReport(null);
                      try {
                        const { data, error } = await supabase.functions.invoke('firecrawl-ai-enrich', { body: { action: 'govt-auto-publish-batch', draft_id: 'batch' } });
                        if (error) throw error;
                        setGovtPublishReport({ published: data?.published || 0, failed: data?.failed || 0, total: data?.total || 0, results: data?.results || [], timestamp: new Date().toLocaleString() });
                        await fetchDrafts();
                      } catch (e: any) { toast({ title: 'Auto-publish failed', description: e.message, variant: 'destructive' }); }
                      finally { setGovtPublishRunning(false); }
                    }}
                  >
                    {govtPublishRunning ? <Loader2 className="h-3.5 w-3.5 animate-spin mr-1.5" /> : <Send className="h-3.5 w-3.5 mr-1.5" />}Auto Publish
                  </Button>
                  <Button
                    variant="outline" size="sm" disabled={govtRetryRunning || loading}
                    onClick={async () => {
                      const retryDrafts = drafts.filter(d => ['retry', 'retry_needed', 'incomplete'].includes(d.publish_readiness || '') && (d.retry_count || 0) < 3);
                      ...
                      for (const d of retryDrafts.slice(0, 20)) {
                        try {
                          const { data, error } = await supabase.functions.invoke('firecrawl-ai-enrich', { body: { action: 'govt-retry-failed', draft_id: d.id, aiModel: selectedModel } });
                  ...
                  >
                    {govtRetryRunning ? <Loader2 className="h-3.5 w-3.5 animate-spin mr-1.5" /> : <RotateCcw className="h-3.5 w-3.5 mr-1.5" />}Retry Failed
                  </Button>
                  <Button
                    variant="outline" size="sm" disabled={govtValidateRunning || loading}
                    onClick={async () => {
                      const govtDrafts = drafts.filter(d => d.status !== 'promoted');
                      if (govtDrafts.length === 0) { toast({ title: 'No govt drafts to validate' }); return; }
                      setGovtValidateRunning(true);
                      let ok = 0;
                      for (const d of govtDrafts) {
                        try { await supabase.functions.invoke('firecrawl-ai-enrich', { body: { action: 'govt-validate-publish', draft_id: d.id } }); ok++; } catch { /* skip */ }
                      }
                  ...
                  >
                    {govtValidateRunning ? <Loader2 className="h-3.5 w-3.5 animate-spin mr-1.5" /> : <CheckCircle className="h-3.5 w-3.5 mr-1.5" />}Validate All
                  </Button>
                </>
              )}
        ...
        <CardContent>
          ...
          {bulkRunning && bulkProgress && (
            <div className="mb-3 p-3 rounded-lg border bg-muted/50 space-y-2">
              <div className="flex items-center justify-between text-sm">
                <span className="font-medium">Processing {bulkProgress.current}/{bulkProgress.total} — <span className="text-muted-foreground">{bulkProgress.currentTitle}</span></span>
                <Button size="sm" variant="ghost" className="h-6 text-xs text-destructive hover:text-destructive" onClick={() => { bulkCancelRef.current = true; }}><X className="h-3 w-3 mr-1" /> Cancel</Button>
              </div>
          ...
          {bulkImageRunning && bulkImageProgress && (
            <div className="mb-3 p-3 rounded-lg border bg-muted/50 space-y-2">
              <div className="flex items-center justify-between text-sm">
                <span className="font-medium">🖼️ Creating images {bulkImageProgress.current}/{bulkImageProgress.total} — <span className="text-muted-foreground">{bulkImageProgress.currentTitle}</span></span>
                <Button size="sm" variant="ghost" className="h-6 text-xs text-destructive hover:text-destructive" onClick={() => { bulkImageCancelRef.current = true; }}><X className="h-3 w-3 mr-1" /> Cancel</Button>
              </div>
          ...
          {bulkFixFieldsRunning && bulkFixFieldsProgress && (
            <div className="mb-3 p-3 rounded-lg border bg-muted/50 space-y-2">
              <div className="flex items-center justify-between text-sm">
                <span className="font-medium">🔧 Fixing fields {bulkFixFieldsProgress.current}/{bulkFixFieldsProgress.total} — <span className="text-muted-foreground">{bulkFixFieldsProgress.currentTitle}</span></span>
                <Button size="sm" variant="ghost" className="h-6 text-xs text-destructive hover:text-destructive" onClick={() => { bulkFixFieldsCancelRef.current = true; }}><X className="h-3 w-3 mr-1" /> Cancel</Button>
              </div>
          ...
          {bulkResults && !bulkRunning && (
            <div className="mb-3 p-3 rounded-lg border bg-muted/30 space-y-2">
              <div className="flex items-center justify-between">
                ...
                <Button size="sm" variant="ghost" className="h-6 text-xs" onClick={() => setBulkResults(null)}>Dismiss</Button>
              </div>
          ...
          {isGovt && govtPublishReport && !govtPublishRunning && (
            <div className="mb-3 p-3 rounded-lg border bg-muted/30 space-y-2">
              <div className="flex items-center justify-between">
                <p className="text-sm font-medium flex items-center gap-1.5"><Send className="h-4 w-4" />Govt Auto-Publish Report — {govtPublishReport.timestamp}</p>
                <Button size="sm" variant="ghost" className="h-6 text-xs" onClick={() => setGovtPublishReport(null)}>Dismiss</Button>
              </div>
                      ...
                      <Tooltip>
                        ...
                        <TooltipContent side="right" className="max-w-[350px]">
                          <div className="text-xs space-y-0.5">
                            ...
                            {(r.warnings || []).length > 0 && (<><p className="font-medium mt-1">Warnings:</p>{(r.warnings || []).map((w: string, j: number) => <p key={j}>⚠ {w}</p>)}</>)}
                          </div>
                        </TooltipContent>
                      </Tooltip>
                    </TooltipProvider>
          ...
          {tpCleanerReport && !tpCleanerRunning && (
            <div className="mb-3 p-3 rounded-lg border bg-muted/30 space-y-2">
              <div className="flex items-center justify-between">
                <p className="text-sm font-medium flex items-center gap-1.5"><ShieldCheck className="h-4 w-4" />TP Cleaner Report — {tpCleanerReport.timestamp}</p>
                <Button size="sm" variant="ghost" className="h-6 text-xs" onClick={() => setTpCleanerReport(null)}>Dismiss</Button>
              </div>
              <div className="flex gap-4 text-xs">
              ...
              {tpCleanerReport.failed > 0 && tpCleanerReport.failedRows.length > 0 && (
                <div className="text-xs space-y-0.5 mt-1">
                  <p className="font-medium text-destructive">Failed rows:</p>
                  {tpCleanerReport.failedRows.map((row, i) => <p key={i} className="text-muted-foreground">• {row}</p>)}
                </div>
              )}
              {tpCleanerReport.failed === 0 && <p className="text-xs text-green-600">All rows cleaned successfully — no third-party traces remain.</p>}
            </div>
          )}
          ...
          {loading ? (
            ...
            <p className="text-center py-8 text-muted-foreground text-sm">No draft jobs found for this filter.</p>
          ) : (
                      ...
                      <TableRow key={draft.id} className={busyRows[draft.id] ? 'opacity-70' : ''} data-state={selectedDraftIds.has(draft.id) ? 'selected' : undefined}>
                        ...
                        <TableCell>
                          <div className="space-y-0.5">
                            ...
                            <div className="flex items-center gap-1">
                              ...
                              {draft.source_bucket && <Badge variant="outline" className="text-[9px]">{draft.source_bucket}</Badge>}
                              <Badge
                                variant="outline"
                                className={`text-[9px] ${
                        ...
                        <TableCell>
                          {draft.source_type_tag === 'government' && draft.publish_readiness ? (
                                  ...
                                  <div className="text-xs space-y-1">
                                    ...
                                    {draft.publish_rejection_reasons && draft.publish_rejection_reasons.length > 0 && (
                                      <div className="border-t pt-1 mt-1">
                                        ...
                                        {draft.publish_rejection_reasons.map((r, i) => <p key={i} className="text-muted-foreground">• {r}</p>)}
                                      </div>
                                    )}
                                    {draft.retry_count > 0 && <p className="text-muted-foreground">Retries: {draft.retry_count}/3</p>}
                                  </div>
                                </TooltipContent>
                              </Tooltip>
                            </TooltipProvider>
                            ...
                            <TooltipProvider>
                              <Tooltip>
                                <TooltipTrigger>
                                  <CircleDot className={`h-4 w-4 ${readiness === 'green' ? 'text-green-500' : readiness === 'yellow' ? 'text-yellow-500' : 'text-red-500'}`} />
                                </TooltipTrigger>
                                <TooltipContent side="right" className="max-w-[250px]"><p className="text-xs">{readinessTooltip}</p></TooltipContent>
                              </Tooltip>
                            </TooltipProvider>
                          )}
                        </TableCell>
                        ...
                        <TableCell>
                          <div className="space-y-0.5">
                            ...
                            {draft.dedup_reason && <p className="text-[9px] text-muted-foreground line-clamp-1" title={draft.dedup_reason}>{draft.dedup_reason}</p>}
                          </div>
                        </TableCell>
                        <TableCell>
                          <div className="text-sm">
                            {draft.fields_extracted}
                            {missingCount > 0 && <span className="text-[10px] text-orange-500 ml-1" title={`Missing: ${draft.fields_missing?.join(', ')}`}>({missingCount} missing)</span>}
                          </div>
                        </TableCell>
                        <TableCell>
                                  ...
                                  <Tooltip key={action}>
                                    <TooltipTrigger>
                                      <span className={`inline-flex items-center gap-0.5 rounded-full border px-1.5 py-0.5 text-[9px] font-medium ${STEP_BADGE_STYLES[state]}`}>
                                        <Icon className={`h-2.5 w-2.5 ${state === 'running' ? 'animate-spin' : ''}`} />{label}
                                      </span>
                                    </TooltipTrigger>
                                    <TooltipContent side="top" className="max-w-[250px]"><p className="text-xs">{tooltip}</p></TooltipContent>
                                  </Tooltip>
                                );
                              })}
                            </TooltipProvider>
                        ...
                        <TableCell>
                          <Badge variant={
                            draft.status === 'promoted' ? 'default' : draft.status === 'approved' ? 'default' :
                            draft.status === 'reviewed' ? 'secondary' : draft.status === 'rejected' ? 'destructive' : 'outline'
                          } className="text-[10px]">{draft.status}</Badge>
                        </TableCell>
                          ...
                          <div className="flex items-center gap-0.5 justify-end">
                            ...
                            <Button
                              ...
                              onClick={async () => {
                                ...
                                try {
                                  const { data, error } = await supabase.functions.invoke('firecrawl-cleanup-branding', { body: { action: 'clean-single', draft_id: draft.id } });
                                  if (error) throw error;
                                  toast({ title: 'TP Cleaner', description: `${data?.status === 'cleaned' ? '✅ Cleaned' : '⚠️ ' + data?.status} — ${data?.traces_found || 0} traces found` });
                                  await fetchDrafts();
                                } catch (e: any) { toast({ title: 'TP Cleaner failed', description: e.message, variant: 'destructive' }); }
                                finally { setBusyRows(prev => { const n = { ...prev }; delete n[draft.id]; return n; }); }
                              }}
                              title={draft.tp_clean_status === 'cleaned' ? 'Already cleaned' : 'Run Third Party Cleaner'}
                              className="gap-0.5 h-7 px-2"
                            >
                              {busyRows[draft.id] === 'tp-clean' ? <Loader2 className="h-3 w-3 animate-spin" /> : <ShieldCheck className="h-3 w-3" />}
                              <span className="text-[10px]">Clean</span>
                            </Button>
                            <Button size="sm" variant="outline" onClick={() => setPreviewDraft(draft)} title="Preview" className="gap-0.5 h-7 px-2">
                              <Eye className="h-3 w-3" /><span className="text-[10px]">Preview</span>
                            </Button>
                            <Button
                              ...
                              title={draft.status === 'promoted' ? 'Already published' : 'Publish'}
                              className="gap-0.5 h-7 px-2"
                            >
                              {publishing ? <Loader2 className="h-3 w-3 animate-spin" /> : <Send className="h-3 w-3" />}
                              <span className="text-[10px]">{draft.status === 'promoted' ? 'Live' : 'Publish'}</span>
                            </Button>
                            <Button
                              size="sm" variant="outline"
                              disabled={!!busyRows[draft.id]}
                              onClick={() => {
                                if (hasExistingImage(draft)) setPreviewImage({ url: draft.cover_image_url!, title: draft.title || 'Untitled' });
                                else createImage(draft.id);
                              }}
                              title={hasExistingImage(draft) ? 'Click to preview cover image' : 'Generate cover image'}
                              className="gap-0.5 h-7 px-2"
                            >
                              {busyRows[draft.id] === 'ai-cover-image' ? <Loader2 className="h-3 w-3 animate-spin" /> :
                               hasExistingImage(draft) ? <Eye className="h-3 w-3 text-green-500" /> : <Image className="h-3 w-3" />}
                              <span className="text-[10px]">{hasExistingImage(draft) ? 'View' : 'Image'}</span>
                            </Button>
                            <Button
                              size="sm" variant="default" disabled={!!busyRows[draft.id]}
                              onClick={() => runAiAction(draft.id, 'ai-run-all')} title="Run All AI Steps" className="gap-0.5 h-7 px-2"
                            >
                              {busyRows[draft.id] === 'ai-run-all' ? <Loader2 className="h-3 w-3 animate-spin" /> : <Zap className="h-3 w-3" />}
                              <span className="text-[10px]">Run All</span>
                            </Button>
                            <DropdownMenu>
                              <DropdownMenuTrigger asChild>
                                <Button size="sm" variant="ghost" disabled={!!busyRows[draft.id]}>
                                  {busyRows[draft.id] && busyRows[draft.id] !== 'ai-run-all' ? <Loader2 className="h-3 w-3 animate-spin" /> : <MoreHorizontal className="h-3 w-3" />}
                                </Button>
                              </DropdownMenuTrigger>
                              <DropdownMenuContent align="end" className="w-52">
                                <DropdownMenuItem onClick={() => updateStatus(draft.id, 'reviewed')}><Eye className="h-3.5 w-3.5 mr-2" /> Mark Reviewed</DropdownMenuItem>
                                <DropdownMenuItem onClick={() => updateStatus(draft.id, 'approved')}><ThumbsUp className="h-3.5 w-3.5 mr-2" /> Approve</DropdownMenuItem>
                                <DropdownMenuItem onClick={() => updateStatus(draft.id, 'rejected')} className="text-destructive"><XCircle className="h-3.5 w-3.5 mr-2" /> Reject</DropdownMenuItem>
                                <DropdownMenuSeparator />
                                {AI_ACTIONS.map(({ action, label, icon: Icon, description }) => (
                                  <DropdownMenuItem key={action} onClick={() => runAiAction(draft.id, action)} disabled={!!busyRows[draft.id]} className="flex items-start gap-2">
                                    <Icon className="h-3.5 w-3.5 mt-0.5 shrink-0" />
                                    <div><p className="text-sm font-medium">{label}</p><p className="text-[10px] text-muted-foreground">{description}</p></div>
                                  </DropdownMenuItem>
                                ))}
                                <DropdownMenuSeparator />
                                <DropdownMenuItem onClick={() => runAiAction(draft.id, 'rollback-ai-action')} disabled={!!busyRows[draft.id]}>
                                ...
                                {draft.official_notification_url && (
                                  <DropdownMenuItem asChild>
                                    <a href={draft.official_notification_url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2">
                                      <ExternalLink className="h-3.5 w-3.5" /><span className="text-sm">Official Link</span>
                                      {draft.official_link_confidence && <Badge variant="outline" className="text-[9px] ml-auto">{draft.official_link_confidence}</Badge>}
                                    </a>
                                  </DropdownMenuItem>
                                )}
                              </DropdownMenuContent>
      ...
      <Dialog open={!!previewImage} onOpenChange={(open) => !open && setPreviewImage(null)}>
        <DialogContent className="max-w-lg">
          ...
          {previewImage && <img src={previewImage.url} alt={previewImage.title} className="w-full rounded-lg object-contain max-h-[70vh]" />}
        </DialogContent>
      </Dialog>

      {/* Draft Preview Dialog */}
      <FirecrawlDraftPreviewDialog draft={previewDraft} open={!!previewDraft} onClose={() => setPreviewDraft(null)} />
        ...
        <AlertDialogContent className="max-w-lg">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription asChild>
              <div className="space-y-3">
                <p className="text-sm"><span className="font-medium">{publishValidation?.draft.title || 'Untitled'}</span>{' — '}{publishValidation?.draft.organization_name || 'Unknown Org'}</p>
                {(publishValidation?.errors.length ?? 0) > 0 && (
                  <div className="bg-destructive/10 rounded-lg p-3 space-y-1">
                    ...
                    {publishValidation?.errors.map((e, i) => <p key={i} className="text-sm text-destructive flex items-start gap-1.5"><XCircle className="h-3.5 w-3.5 mt-0.5 shrink-0" /> {e}</p>)}
                  </div>
                )}
                {(publishValidation?.warnings.length ?? 0) > 0 && (
                  <div className="bg-yellow-50 dark:bg-yellow-900/20 rounded-lg p-3 space-y-1">
                    ...
                    {publishValidation?.warnings.map((w, i) => <p key={i} className="text-sm text-yellow-600 dark:text-yellow-400 flex items-start gap-1.5"><AlertTriangle className="h-3.5 w-3.5 mt-0.5 shrink-0" /> {w}</p>)}
                  </div>
                )}
          ...
          <AlertDialogFooter>
            ...
            {publishValidation && publishValidation.errors.length === 0 && (
              <AlertDialogAction onClick={() => publishValidation && executePublish(publishValidation.draft)} disabled={publishing}>
