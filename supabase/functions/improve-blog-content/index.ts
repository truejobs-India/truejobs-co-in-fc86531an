...
const SUPPORTED_MODELS = ['gemini', 'gemini-flash', 'gemini-pro', 'mistral', 'claude-sonnet', 'claude', 'openai', 'gpt5', 'gpt5-mini', 'groq', 'lovable-gemini', 'vertex-flash', 'vertex-pro', 'vertex-3.1-pro', 'vertex-3-flash', 'vertex-3.1-flash-lite', 'nova-pro', 'nova-premier', 'nemotron-120b', 'azure-gpt4o-mini', 'azure-gpt41-mini', 'azure-deepseek-v3', 'azure-deepseek-r1'];
...
async function callAI(aiModel: string, prompt: string, maxTokens: number, options?: { systemPrompt?: string }): Promise<{ raw: string; finishReason: string; actualProvider: string; actualModelId: string; usage?: { inputTokens?: number; outputTokens?: number } }> {
  ...
  switch (model) {
    ...
    case 'azure-deepseek-v3': {
      ...
      const text = await callAzureDeepSeek(prompt, { maxTokens, temperature: 0.5 });
      resultJson = JSON.stringify({ __raw: text, __finishReason: 'stop' });
      actualProvider = 'azure-deepseek'; actualModelId = 'DeepSeek-V3.1'; break;
    }
    case 'azure-deepseek-r1': {
      const { callAzureDeepSeek } = await import('../_shared/azure-deepseek.ts');
      const text = await callAzureDeepSeek(prompt, { model: 'DeepSeek-R1', maxTokens, temperature: 0.5 });
      resultJson = JSON.stringify({ __raw: text, __finishReason: 'stop' });
